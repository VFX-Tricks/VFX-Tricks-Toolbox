...
class HoudiniPlugin (DeadlinePlugin):
    ...
    def RenderArgument(self):
        ...
        hrender = Path.Combine( self.GetPluginDirectory(),"hrender_dlVFXTricks.py" )
        arguments = [ "\"%s\"" % hrender ]
        ...
        # nde
        try:
            node = self.GetPluginInfoEntryWithDefault("Node", None)
            if node:
                arguments.append( "-node %s" % (node) )
        except:
            pass

        # add JobType Plugin info, for some reason "jobType" arg doesnt work...
        try:
            type = self.GetPluginInfoEntryWithDefault("Type", None)
            if type:
                arguments.append( "-type %s" % (type) )
        except:
            pass

        # add Section Plugin info
        try:
            section = self.GetPluginInfoEntryWithDefault("Section", None)
            if section:
                arguments.append( "-section %s" % (section) )
        except:
            pass
    ...
    def HandleStdoutFrameProgress(self):
        ...
        elif self.ropType in ("arnold", "geometry", "rop_geometry", "wedge", "Octane_ROP", "ris::22") or "VFXTricks::cacher" in self.ropType:
